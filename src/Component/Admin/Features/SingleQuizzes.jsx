...
const SingleQuizzes = ({ quizzes }) => {
  const { id, question, video_id, video_title } = quizzes || {};
  // local state
  ...
  useEffect(() => {
    if (isSuccess) {
      toast("Successfully delete this quiz.");
    }
  }, [isSuccess]);
  return (
    <tr>
      <td className="table-td">
        Quiz-{video_id} -{" "}
        {`${question.split(" ").splice(0, 10).join(" ")}
          ${question.split(" ").length > 10 ? "........" : ""}`}
      </td>
      <td className="table-td">{`${video_title
        .split(" ")
        .splice(0, 10)
        .join(" ")}
          ${video_title.split(" ").length > 10 ? "........" : ""}`}</td>
